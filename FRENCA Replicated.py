...
def train_frenca_safe(model, dataloader, epochs=3, lr=5e-4):
    """Safe FRENCA training with checkpoints and error handling."""

    print("--- Phase 1: Training high-computing user with BS encoder ---")

    # Setup for phase 1
    high_params = list(model.source_encoder.parameters()) + \
                  list(model.channel_encoder.parameters()) + \
                  list(model.channel_decoder_high.parameters()) + \
                  list(model.source_decoder_high.parameters())

    optimizer_high = optim.AdamW(high_params, lr=lr, weight_decay=1e-5)
    #scheduler_high = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer_high, T_max=epochs-WARMUP_EPOCHS, eta_min=1e-6)
    #scheduler_high = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer_high, mode='min', factor=0.2, patience=10)
    #criterion = PerceptualLoss(alpha=0.8, device=device).to(device)
    criterion = nn.MSELoss().to(device)

    # Try to load checkpoint
    checkpoint_path = CHECKPOINT_DIR / "phase1_checkpoint.pt"
    start_epoch, best_loss = load_checkpoint(checkpoint_path, model, optimizer_high)
    ...
    for epoch in range(start_epoch, epochs):
        ...
        try:
            for batch_idx, images in enumerate(dataloader):
                ...
                snr = torch.FloatTensor(np.random.choice([1, 3, 5, 7], size=batch_size)).to(device)

                optimizer_high.zero_grad()

                # Safe training step
                loss, _ = safe_train_step(model, images, snr, criterion, 'high')

                loss.backward()
                torch.nn.utils.clip_grad_norm_(high_params, max_norm=1.0)  # Gradient clipping
                optimizer_high.step()
                ...
                if batch_idx % 10 == 0:
                    print(f'Phase 1 - Epoch {epoch+1}/{epochs}, Batch {batch_idx+1}/{len(dataloader)}, Loss: {loss.item():.4f}')
        ...
        print(f'Phase 1 - Epoch {epoch+1} completed. Average Loss: {avg_loss:.4f}')

        #Save checkpoint
        if avg_loss < best_loss:
            best_loss = avg_loss
            save_checkpoint(model, optimizer_high, epoch + 1, avg_loss, checkpoint_path)
       ...
       # model.eval()
        ...
        #with torch.no_grad(): # Disable gradient calculation
            #for images in validation_dataloader:
                    ...
                    # Use a fixed, representative SNR for consistent validation
                    #snr = torch.FloatTensor([5.0]).expand(batch_size).to(device) # Medium SNR of 5dB

                    # Perform a forward pass for the high-capacity user
                    #reconstructed = model(images, snr, user_type='high', add_noise=True)
        ...
        #scheduler_high.step(avg_validation_loss)



        print(f"\nEpoch {epoch+1} Summary:")
        print(f"  -> Avg Train Loss: {avg_loss:.6f}")
        #print(f"  -> Avg Validation Loss: {avg_validation_loss:.6f}")
        #print(f"  -> Current LR: {optimizer_high.param_groups[0]['lr']}\n")
        print(f"  -> Best Loss: {best_loss}")

        #if avg_validation_loss < best_loss:
                #print(f"Validation loss improved from {best_loss:.6f} to {avg_validation_loss:.6f}...")
                #best_loss = avg_validation_loss
                #save_checkpoint(model, optimizer_high, epoch + 1, best_loss, CHECKPOINT_DIR / "main_checkpoint.pt")

    # Phase 2: Train low-computing user
    print("\n--- Phase 2: Training low-computing user with frozen encoder ---")

    # Freeze encoder and high-capacity decoder parts
    for param in model.parameters():
        param.requires_grad = False

    # Unfreeze only the low-capacity decoder parts
    for param in model.channel_decoder_low.parameters():
        param.requires_grad = True
    for param in model.source_decoder_low.parameters():
        param.requires_grad = True

    low_params = list(model.channel_decoder_low.parameters()) + \
                 list(model.source_decoder_low.parameters())

    optimizer_low = optim.AdamW(low_params, lr=lr, weight_decay=1e-5)
    #scheduler_low = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer_low, T_max=epochs-WARMUP_EPOCHS, eta_min=1e-6)
    #scheduler_low = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer_high, mode='min', factor=0.2, patience=10, verbose=True)

    # Try to load phase 2 checkpoint
    checkpoint_path_phase2 = CHECKPOINT_DIR / "phase2_checkpoint.pt"
    start_epoch, best_loss = load_checkpoint(checkpoint_path_phase2, model, optimizer_low)
    ...
    for epoch in range(start_epoch, epochs):
        ...
        try:
            for batch_idx, images in enumerate(dataloader):
                ...
                # Safe training step
                loss, _ = safe_train_step(model, images, snr, criterion, 'low')

                loss.backward()
                torch.nn.utils.clip_grad_norm_(low_params, max_norm=1.0)
                optimizer_low.step()
                ...
                if batch_idx % 10 == 0:
                    print(f'Phase 2 - Epoch {epoch+1}/{epochs}, Batch {batch_idx+1}/{len(dataloader)}, Loss: {loss.item():.4f}')
        ...
        avg_loss = total_loss / max(num_batches, 1)
        print(f'Phase 2 - Epoch {epoch+1} completed. Average Loss: {avg_loss:.4f}')

        # Save checkpoint
        if avg_loss < best_loss:
            best_loss = avg_loss
            save_checkpoint(model, optimizer_low, epoch + 1, avg_loss, checkpoint_path_phase2)
